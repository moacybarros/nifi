...
public class CouchbaseMapCacheClient extends AbstractControllerService implements AtomicDistributedMapCacheClient {
    ...
    public void configure(final ConfigurationContext context) {
        ...
        final String bucketName = context.getProperty(BUCKET_NAME).getValue();
    ...
    private class CouchbaseCacheEntry<K, V> implements CacheEntry<K, V> {
        private final K key;
        private final V value;
        private final long revision;

        public CouchbaseCacheEntry(K key, V value, long revision) {
            this.key = key;
            this.value = value;
    ...
    public <K, V> CacheEntry<K, V> fetch(K key, Serializer<K> keySerializer, Deserializer<V> valueDeserializer) throws IOException {
        ...
        return new CouchbaseCacheEntry<>(key, value, doc.cas());
    }
    ...
    public <K, V> boolean replace(K key, V value, Serializer<K> keySerializer, Serializer<V> valueSerializer, long revision) throws IOException {
        final String docId = toDocumentId(key, keySerializer);
        final Document doc = toDocument(docId, value, valueSerializer, revision);
        try {
            if (revision < 0) {
