...
public class CouchbaseKeyValueLookupService extends AbstractCouchbaseLookupService implements StringLookupService {
    ...
    public void onEnabled(final ConfigurationContext context) throws InitializationException {
        ...
        subDocPath = context.getProperty(LOOKUP_SUB_DOC_PATH).getValue();
    }

    @Override
    public Optional<String> lookup(Map<String, String> coordinates) throws LookupFailureException {

        final Bucket bucket = couchbaseClusterService.openBucket(bucketName);
        final Optional<String> docId = Optional.ofNullable(coordinates).map(c -> c.get(KEY));

        if (StringUtils.isBlank(subDocPath)) {
            return docId.map(key -> bucket.get(key, RawJsonDocument.class))
